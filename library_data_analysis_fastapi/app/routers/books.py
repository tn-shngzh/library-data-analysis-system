...
@router.get("/stats")
async def get_book_stats(conn=Depends(get_db)):
...
async def get_book_categories(conn=Depends(get_db)):
    with conn.cursor() as cur:
        cur.execute("""
...
@router.get("/hot")
async def get_hot_books(conn=Depends(get_db)):
    with conn.cursor() as cur:
        cur.execute("""
...
async def get_categories_list(conn=Depends(get_db)):
    with conn.cursor() as cur:
        cur.execute("""
