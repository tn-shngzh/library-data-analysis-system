"""
Safe redistribution avoiding deadlocks - one borrower at a time, one commit
"""
import os
import sys
...
print("\n[2/4] Redistributing borrower records...")
cur.execute("""
...
for borrower_id, cnt in over_borrowers:
    ...
    for i in range(0, len(ids), 5000):
        chunk = ids[i:i+5000]
        new_ids = [random.choice(under_borrowers) for _ in chunk]
        
        cur.execute("""
            UPDATE circulations SET borrower_id = v.new_id
            FROM (VALUES %s) AS v(id, new_id)
            WHERE circulations.id = v.id
        """ % ','.join(f"({rid},{nid})" for rid, nid in zip(chunk, new_ids)))
        
        total_updated += len(chunk)

print(f"  Updated {total_updated:,} borrower records")

print("\n[3/4] Redistributing book records...")
cur.execute("""
...
for bib_id, cnt in over_books:
    ...
    for i in range(0, len(ids), 5000):
        ...
        cur.execute("""
            UPDATE circulations SET bib_id = v.new_id
            FROM (VALUES %s) AS v(id, new_id)
            WHERE circulations.id = v.id
        """ % ','.join(f"({rid},{nid})" for rid, nid in zip(chunk, new_ids)))
...
if over_c and under_c:
    ...
    for i in range(0, len(to_move), 1000):
        ...
        vals = ','.join(f"({bib},'{under_list[i+j % len(under_list)]}')" for j, bib in enumerate(chunk))
        cur.execute(f"""
            UPDATE book_categories bc SET category = v.nc
            FROM (VALUES {vals}) AS v(bib, nc)
            WHERE bc.bib_id = v.bib
        """)
